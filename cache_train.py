...
def set_cache(cache_dict, key, value):
    cache_dict[key] = (value, time())
...
async def search_books(
    ...
    cache_key = f"{q}-{skip}-{limit}"
    cached = get_from_cache(books_cache, cache_key)
    if cached:
        return cached
    ...
    set_cache(books_cache, cache_key, result)
    return result
...
async def search_authors(
    ...
    cached = get_from_cache(authors_cache, q)
    if cached:
        return cached
    ...
    result = {"query": q, "results": final_results}

    set_cache(authors_cache, q, result)
    return result
...
async def update_book_part(
    ...
    cursor.execute(
        "SELECT title, author, publisher, first_publish_year, image_url FROM books WHERE id = %s",
        (id,),
    )
    existing = cursor.fetchone()
    if not existing:
    ...
    if image:
        ...
        with open(new_path, "wb") as buffer:
            shutil.copyfileobj(image.file, buffer)
        if current_image:
            old_path = os.path.join("images", current_image)
            if os.path.exists(old_path):
                os.remove(old_path)
